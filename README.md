...
Проект представляет собой пять мини-игр для поддержания мозговой активности.

## 🎮 Описание игр

| Игра | Суть |
|------|------|
| **Проверка на чётность** | Пользователю показывается случайное число. Нужно ответить `yes`, если число чётное, или `no` — если нечётное. |
...
| **НОД** | Пользователю показываются два случайных числа (например, `25 50`). Нужно вычислить и ввести наибольший общий делитель этих чисел. |
| **Арифметическая прогрессия** | Показывается ряд чисел, образующий арифметическую прогрессию, где одно из чисел заменено двумя точками. Игрок должен определить пропущенное число. |
| **Простое ли число?** | Игроку требуется определить, является ли показанное число простым. |
